...
class EquityEnv(gym.Env):
    ...
    def reset(self):
        ...
        """
        if self.split_data:
            self.start_idx = random.choice(self.train_period[: len(self.train_period) - self.episode_length + 1])
            self.end_idx = self.start_idx + self.episode_length
        else:
            if self.test_option == "train":
                self.start_idx = self.train_period[0]
                self.end_idx = self.train_period[-1]
            elif self.test_option == "develop":
                self.start_idx = self.develop_period[0]
    ...
    def _setup(self):
        states_df = pd.read_csv(DATA_DIR + "/state.csv", index_col=0, parse_dates=True)
        prices_df = pd.read_csv(DATA_DIR + "/price.csv", index_col=0, parse_dates=True)
